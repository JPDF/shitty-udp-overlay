...
int main(int argc, char **argv) {
	...
	srand(time(NULL));
	int timer = 0, resendCount = 0, seq = 0, windowsize = MAX_WINDOWSIZE, dataIndex = 0, acksReceived;
	...
	while (1) {
		...
		switch (state) {
			...
			case ACK_SENT:
				...
				break;
			case WAIT:
				if (dataIndex == MAX_DATA - 1 && acksReceived == MAX_DATA) {
					createPacket(&packet, FIN, 0, currentSeq, windowsize, NULL);
					sendPacket(mySocket, &packet, &otherAddress, 0);
					addPacketTimer(&timerList, &packet, &otherAddress, deltaTime, TIMEOUT);
					state = FIN_WAIT_1;
				}
				else if (dataIndex != MAX_DATA && currentSeq != slidingWindowIndexLast) { // SEND DATA
					createPacket(&packet, FRAME, 0, currentSeq, windowsize, data[dataIndex]);
					sendPacket(mySocket, &packet, &otherAddress, 0);
					addPacketTimer(&timerList, &packet, &otherAddress, deltaTime, TIMEOUT);
					currentSeq = (seq + 1) % MAX_SEQUENCE;
					dataIndex++;
				}
